...
DB_PATH = os.path.join(BASE_DIR, "physics_chats.db")
...
def load_physics_questions():
    path = os.path.join(os.path.dirname(__file__), "static", "physics_qa.json")
...
physics_questions = load_physics_questions()
...
def get_answer_to_question(query):
    ...
    questions = [normalize_text(item['question']) for item in physics_questions]
    ...
    if closest:
        for item in physics_questions:
            if normalize_text(item['question']) == closest[0]:
                return item['answer']
    return "क्षमस्व, या प्रश्नाचं उत्तर सापडलं नाही."
...
def process_speech():
    ...
    answer = "क्षमस्व, आवाज समजला नाही."
    if query != "None":
...
@app.route("/physics/clear_history", methods=["POST"])
def clear_history():
    username = session.get('username', 'guest')
...
@app.route("/physics/download/csv")
def download_csv():
    username = session.get('username', 'guest')
...
@app.route("/physics/download/pdf")
def download_pdf():
    ...
    y = height - 30
    c.drawCentredString(width / 2, y, f"{username}'s Chat History")
    y -= 20
