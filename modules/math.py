...
DB_PATH = os.path.join(BASE_DIR, "math_chats.db")
...
def load_math_questions():
    path = os.path.join(os.path.dirname(__file__), "static", "math.json")
...
def get_answer_to_question(query):
    ...
    questions = [normalize_text(item['question']) for item in math_questions]
...
@app.route("/math/clear_history", methods=["POST"])
def clear_history():
    username = session.get('username', 'guest')
...
def download_pdf():
    ...
    y = height - 30
    c.drawCentredString(width / 2, y, f"{username}'s Math Chat History")
    y -= 20
