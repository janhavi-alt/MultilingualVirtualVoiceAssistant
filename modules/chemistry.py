...
import pytz
from flask import Flask, render_template, jsonify, session, request, redirect, url_for, send_file, make_response
...
DB_PATH = os.path.join(BASE_DIR, "chemistry_chats.db")
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{DB_PATH}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

db = SQLAlchemy(app)

# Define ChatHistory table for Chemistry
...
def load_chemistry_questions():
    path = os.path.join(os.path.dirname(__file__), "static", "chemistry_qa.json")
...
chemistry_questions = load_chemistry_questions()
...
def get_answer_to_question(query):
    ...
    questions = [normalize_text(item['question']) for item in chemistry_questions]
...
@app.route("/chemistry/clear_history", methods=["POST"])
def clear_history():
    username = session.get('username', 'guest')
...
@app.route("/chemistry/download/csv")
def download_csv():
    username = session.get('username', 'guest')
...
@app.route("/chemistry/download/pdf")
def download_pdf():
    ...
    y = height - 30
    c.drawCentredString(width / 2, y, f"{username}'s Chemistry Chat History")
    y -= 20
